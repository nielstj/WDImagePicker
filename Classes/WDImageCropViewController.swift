...
internal class WDImageCropViewController: UIViewController {
    ...
    private func setupNavigationBar() {
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .cancel,
            target: self, action: #selector(actionCancel(_:)))
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Use", style: .plain,
            target: self, action: #selector(actionUse(_:)))
    }
    ...
    private func setupCancelButton() {
        ...
        self.cancelButton.addTarget(self, action: #selector(actionCancel(_:)), for: .touchUpInside)
    }
    ...
    private func setupUseButton() {
        ...
        self.useButton.setTitleShadowColor(
            UIColor(red: 0.118, green: 0.247, blue: 0.455, alpha: 1), for: UIControlState())
        self.useButton.addTarget(self, action: #selector(actionUse(_:)), for: .touchUpInside)
    }
